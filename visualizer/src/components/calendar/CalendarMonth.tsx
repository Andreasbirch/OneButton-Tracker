...
import { groupBy, getWeek } from './helpers';
...
const data = mockdatapresses.map(o => ({timestamp: new Date(o.timestamp), duration: o.duration})).filter(o => o.timestamp.getMonth() === 1);
const groups = groupBy(data, o => o.timestamp.toISOString().split('T')[0]);

// Function to get color based on the count
export function getColor(date: Date): string | null {
    if(!date) 
        return null;
...
function CalendarMonth({year, month}: {year: number, month: number}) {
    var calendar = GetCalendarMonth(year, month);
        ...
        {Object.keys(calendar).map(k => 
            <div className='calendar-week' key={k}>
                ...
                <div className='week-days'>
                    ...
                    <div 
                        className='week-day'
                        style={{backgroundColor: getColor(o)?? ""}}
                        key={`${k}-${i}`}>{o?.getDate()}
                    </div>)}
                </div>
            </div>
