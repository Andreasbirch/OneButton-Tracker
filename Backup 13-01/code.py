...
logger = BoardLogger(headers=['timestamp', 'acc_magnitude', 'stability', 'tilt_x', 'tilt_y', 'button'])
...
# Once every 15 mins, log battery voltage
if alarm.sleep_memory[0]:
    if alarm.sleep_memory[0] != timestamp.tm_min // 15:
        alarm.sleep_memory[0] = timestamp.tm_hour // 15
...
if isinstance(wakealarm, alarm.time.TimeAlarm):
    ...
    time.sleep(.2)
    alarm.exit_and_deep_sleep_until_alarms(x_alarm, y_alarm, button_alarm)
else:
    print("Woke from movement")
    time.sleep(.2)
    alarm.exit_and_deep_sleep_until_alarms(alarm.time.TimeAlarm(monotonic_time=time.monotonic() + 5), button_alarm)